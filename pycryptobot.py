...
def getAction(now: datetime = datetime.today().strftime('%Y-%m-%d %H:%M:%S'), app: PyCryptoBot = None, price: float = 0,
    ...
    if ema12gtema26co is True \
            and (macdgtsignal is True ) \
            and (goldencross is True ) \
            and (obv_pc > -5 ) \
            and (elder_ray_buy )  :
...
def save_to_file(signal):
    with open(os.path.dirname(os.path.realpath(__file__))+'signal', 'w') as filetowrite:
        filetowrite.write(str(signal))
...
def executeJob(sc=None, app: PyCryptoBot = None, state: AppState = None, trading_data=pd.DataFrame()):
    ...
    if len(df_last) > 0:
        ...
        if app.isSimulation():
            ...
            #print("masoudddddddddd", truncate(float(df_last['signal'].values[0])))
            list(map(s.cancel, s.queue))
            s.enter(15, 1, executeJob, (sc, app, state))
