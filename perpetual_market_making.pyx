...
cdef class PerpetualMarketMakingStrategy(StrategyBase):
    ...
    def __init__(self,
        ...
        self._bid_spread = Decimal('1.2') / Decimal('100')
        self._ask_spread = Decimal('1.2') / Decimal('100')
    ...
    def bid_spread(self) -> Decimal:
        ...
        return Decimal('1.2') / Decimal('100')
    ...
    my_signal=''
    def get_signal(self):
        with open(os.path.dirname(os.path.realpath(__file__))+'/signal', "r") as f:
            
            data = f.read()
            self.my_signal=data
            self.logger().warning(f"signal: {data}")
        return float(data)
    ...
    def get_orders(self):
        ...
        if(self.my_signal=='sell'):
            buy_ratio=0.1

        for idx in self.active_positions.values():
            is_buy = True if idx.amount > 0 else False
            self.logger().warning("is_buy: "+str(is_buy))
            self.logger().warning("position_side: "+idx.position_side.name)
        ...
        if buy_ratio >= 1 :
            ...
            self._sell_levels = 1
            orders["order_1"] = ["buy", Decimal(buy_ratio), self._order_amount]
        ...
        if sell_ratio >= 1 :
            ...
            self._sell_levels=1
            orders["order_2"] = ["sell", Decimal(sell_ratio), self._order_amount]
        return orders
    # ---------------------------------------------------------------
        ...
        try:
            ...
            if len(session_positions) == 0:
                ...
                if self._create_timestamp <= self._current_timestamp:
                    self._order_override=self.get_orders()
    ...
    cdef c_apply_order_price_modifiers(self, object proposal):
        self.logger().info(f"masoud,c_apply_order_price_modifiers.")
    ...
    cdef c_apply_order_optimization(self, object proposal):
        ...
        self.logger().info(f"masoud,c_apply_order_optimization.")
    ...
    cdef c_did_complete_buy_order(self, object order_completed_event):
        ...
        self.logger().info(f"masoud,c_did_complete_buy_order.")
